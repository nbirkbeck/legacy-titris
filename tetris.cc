...
void Draw_Text_GDI(const char* message, int x, int y, const RGB& rgb,
                   SDL_Surface* surface) {
...
void outputBlox(FILE* file) {
  ...
  while (i < MAX_BLOCKS) {
    fprintf(file, "x:%d y:%d valid:%d img:%d\n", blocks[i].x, blocks[i].y,
            blocks[i].valid, blocks[i].blockImgIndex);
    i++;
  }
}
void resetStats() {
...
int drawBlock(BLOCK* aBlock) {
  ...
  if (aBlock->y < 0)
    return 1;
  DDraw_Draw_Surface(blockSurf[aBlock->blockImgIndex], x, y, 16, 16, surface,
                     0);
  return 1;
}
      ...
      if (flag) {
        ...
        for (int i = 0; i < 4; i++) {
          if (titris[focal.cur].blocks[i]->y >= 0) {
            gameSurf[titris[focal.cur].blocks[i]->y]
                    [titris[focal.cur].blocks[i]->x] =
...
void insertingNewScore() {
  ...
  if (gBuffer[gNumChars - 1] != '\r') {
    gBuffer[gNumChars] = '\0';
    Draw_Text_GDI("You have new High Score, Enter Name:", 200, 275,
                  RGB(200, 100, 200), surface);
...
int drawNextTitri() {
  ...
  while (i < 4) {
    ...
    DDraw_Draw_Surface(blockSurf[aBlock[i]->blockImgIndex], x, y, 16, 16,
                       surface, 0);
    i++;
  }
  return 1;
